...
public class EnemyController : EnemyBase
{
    ...
    public float horizontalDistance = 10f; // Khoảng cách bay vào trong
    public float amplitude = 3f;           // Biên độ cong
    ...
    void Start()
    {
        float startX = fromLeft ? -horizontalDistance : horizontalDistance;
        startPosition = new Vector3(startX, Random.Range(-3f, 3f), 0);
    ...
    void Update()
    {
        ...
        if (t > 1f)
        {
            Destroy(gameObject); // Di chuyển xong thì huỷ
            return;
        }

        float x = Mathf.Lerp(startPosition.x, 0f, t);
        float y = amplitude * Mathf.Sin(t * Mathf.PI); // Đường cong sin
